...
public class BuysEditorActivity extends AppCompatActivity implements
    ...
    private EditText mID3EditText;
    private EditText mID4EditText;


    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_editor_buys);
        ...
        mID1EditText = (EditText) findViewById(R.id.edit_buys_customer);
        mID2EditText = (EditText) findViewById(R.id.edit_buys_movie);
        mID3EditText = (EditText) findViewById(R.id.edit_buys_shows);
        mID4EditText = (EditText) findViewById(R.id.edit_buys_theatres);

        mID1EditText.setOnTouchListener(mTouchListener);
        mID2EditText.setOnTouchListener(mTouchListener);
        mID3EditText.setOnTouchListener(mTouchListener);
        mID4EditText.setOnTouchListener(mTouchListener);
...
//    }
    ...
    private void saveMovie() {
        ...
        String id3 = mID3EditText.getText().toString().trim();
        String id4 = mID4EditText.getText().toString().trim();
        // Read from input fields
        // Use trim to eliminate leading or trailing white space
...
//        }
        ...
        values.put(MovieContract.MovieEntry._IDCUSTOMER,length1);
        int length3 = Integer.parseInt(id3);
        int length4 = Integer.parseInt(id4);

        int length2 = 0;
        if (!TextUtils.isEmpty(id2)) {
            length2 = Integer.parseInt(id2);
        }
        values.put(MovieContract.MovieEntry._IDMOVIES, length2);
        values.put(MovieContract.MovieEntry._IDshows, length3);
        values.put(MovieContract.MovieEntry._IDtheatres, length4);
        ...
        if (mCurrentMovieUri == null) {
            ...
            Uri newUri = getContentResolver().insert(MovieContract.MovieEntry.CONTENT_URI5, values);
...
// Show a dialog that notifies the user they have unsaved changes
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            ...
            int lengthColumnIndex = cursor.getColumnIndex(MovieContract.MovieEntry._IDCUSTOMER);
            int idColumnIndex = cursor.getColumnIndex(MovieContract.MovieEntry._IDMOVIES);
            int length1ColumnIndex = cursor.getColumnIndex(MovieContract.MovieEntry._IDshows);
            int id1ColumnIndex = cursor.getColumnIndex(MovieContract.MovieEntry._IDtheatres);

            // Extract out the value from the Cursor for the given column index
            int id1 = cursor.getInt(lengthColumnIndex);
            int id2 = cursor.getInt(idColumnIndex);
            int id3 = cursor.getInt(length1ColumnIndex);
            int id4 = cursor.getInt(id1ColumnIndex)
                    ;
            // Update the views on the screen with the values from the database
            mID1EditText.setText(Integer.toString(id1));
            mID2EditText.setText(Integer.toString(id2));
            mID3EditText.setText(Integer.toString(id3));
            mID4EditText.setText(Integer.toString(id4));
